...
def run_image_recognition(context: ScriptRunContext) -> ImageDebugResult:
    ...
    try:
        ...
        while not context.control.stop_requested():
            ...
            iterations += 1
            if not _sleep_with_control(context, 0.5):
                break
...
def run_coordinate_detector(context: ScriptRunContext) -> ImageDebugResult:
    ...
    try:
        ...
        anchor_miss_count = 0
        matcher = TemplateMatcher(capture=capture, logger=context.logger)
        log_important(context.logger, "[检测坐标] 开始检测，容忍度=%.3f", threshold)
        ...
        while not context.control.stop_requested():
            ...
            matches_by_name = matcher.match_groups(groups, region, limit=MAX_DISPLAY_MATCHES)
            anchor_matches = matches_by_name.get("MapAnchor.bmp", [])[:10]
            if not anchor_matches:
                anchor_miss_count += 1
                if last_anchor is not None and anchor_miss_count < 3:
                    anchor = last_anchor
            ...
            people = matches_by_name.get("Me.bmp", [])
            teleports = matches_by_name.get("Teleport.bmp", [])
            runes = matches_by_name.get("Rune.bmp", [])
            ...
            context.logger.info(
                "[检测坐标] people=%s teleports=%s runes=%s anchor=(%s,%s) anchor_status=%s",
                len(people),
                len(teleports),
                len(runes),
                anchor.x,
